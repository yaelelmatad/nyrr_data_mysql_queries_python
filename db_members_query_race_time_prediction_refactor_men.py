...
def main():
    ...
    progFile = "progress.dat"
    pfle = open(progFile, 'wr')
    pfle.close()

    memberList = []
    for memNum in range(0,125000):
        if memNum%progUpdate == 0:
            pfle = open(progFile, 'a')
            pfle.write("At member " + str(memNum) + "\n")
            pfle.close()
        cursor.execute("SELECT pacePerMile, distMiles, memberNumber, date FROM nyrrMemberData WHERE sex = 'm' and memberNumber = " + str(memNum))
